...
const SHELL_CACHE = "ahso-crm-shell-v2";

// Only pre-cache the web app manifest. Static JS/CSS chunks are cached on-demand below.
...
self.addEventListener("fetch", (event) => {
  ...
  if (url.origin === self.location.origin) {
    ...
    // All other same-origin requests (HTML, JSON, etc.) — pass through to network.
    return;
  }
});
